...
train_path = os.path.join('training.txt')
...
def getfive(xs,ys,ts, idxx):
    ...
    dt = ts[idxx[-1]] - ts[idxx[0]]
    dx = xs[idxx[-1]] - xs[idxx[0]]
    dy = ys[idxx[-1]] - ys[idxx[0]]
    mt = ts[-1]
...
def get_feature(df):
    ...
    x_xs = np.array([point[0][0] for point in points])
    y_ys = np.array([point[0][1] for point in points])
    time_ts = np.array([point[1] for point in points])
    ...
    df['t_init'] = ts.values[0]
    df['t_last'] = ts.values[-1]
    df['t_std'] = time_ts.std()
    df['dx'] = dx
    df['dy'] = dy
    df['-dx'] = -1 * dx
    df['-dy'] = -1 * dy
    ...
    xyt = getfive(x_xs,y_ys,time_ts, idxx)
    df['mid_xyt1'] = xyt[0]
    df['mid_xyt2'] = xyt[1]
    df['mid_xyt3'] = xyt[2]
    ...
    twz = gettoward(x_xs,y_ys,time_ts)  # 12,13 mid five points x,y speed
    df['twz1'] = twz.tolist()[0]
    df['twz2'] = twz.tolist()[1]
    df['twz3'] = twz.tolist()[2]
    df['twz4'] = twz.tolist()[3]
    df['twz5'] = twz.tolist()[4]
    df['twz6'] = twz.tolist()[5]
    df['twz7'] = twz.tolist()[6]
    df['twz8'] = twz.tolist()[7]
    df['twz9'] = twz.tolist()[8]

    twz = getplr(x_xs,y_ys,time_ts)
    df['plr1'] = twz.tolist()[0]
    df['plr2'] = twz.tolist()[1]
    df['plr3'] = twz.tolist()[2]
    df['plr4'] = twz.tolist()[3]
    df['plr5'] = twz.tolist()[4]
    df['plr6'] = twz.tolist()[5]
